...
class DataProcessor:
    ...
    def process_sentence(self, sen):
        ...
        for pred in sen.get_predicates():
            ...
            for pos, word in enumerate(sen):
                ...
                pred_index = self.get_index(pred.arguments[pos], self.labels)
                entity_index = self.get_index(word.entity_tag, self.labels)
                this_pred.append((word_index, int(word == pred), pred_index, entity_index))
    ...
    def process_sentence_not_in_vob(self, sen, is_train=True):
        dt = []
        for pred in sen.get_predicates():
            this_pred = []
            for pos, word in enumerate(sen):
                ...
                if is_train:
                    ...
                    this_pred.append((word_index, int(word == pred), pred_index))
