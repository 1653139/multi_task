...
class DSRL:
    def __init__(self, config_path="config/srl.ini", tag_id=None):
        ...
        self.srl_targets = tf.placeholder(dtype=tf.int32, shape=[None, None], name='srl_targets')  # target labels
        self.ner_targets = tf.placeholder(dtype=tf.int32, shape=[None, None], name='ner_targets')
    ...
    #  this function is only used for third-parties
    ...
    @lazy_property
    def loss(self):
        def loss_crf(targets):
            print('loss crf')
            log_likelihood, trans_params = tf.contrib.crf.crf_log_likelihood(
                self.logits, targets, self.seq_lens)
            self.trans_params = trans_params  # need to evaluate it for decoding
            loss = tf.reduce_mean(-log_likelihood)
            tf.summary.scalar("loss", loss)
            return loss
        def loss_softmax(targets):
            print('loss softmax')
            if self.cfg.label_smoothing != 0.0:
                ...
                q = label_smoothing / n
                # Soft targets.
                soft_targets = tf.one_hot(targets, depth=len(self.pp.labels), axis=-1, on_value=p, off_value=q)
                losses = tf.nn.softmax_cross_entropy_with_logits(logits=self.logits, labels=soft_targets)
            else:
                losses = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.logits, labels=targets)
        ...
        if self.cfg.is_multi_task:
            srl_loss = loss_crf(self.srl_targets) if self.cfg.main_task == 'crf' else loss_softmax(self.srl_targets)
            ner_loss = loss_crf(self.ner_targets) if self.cfg.auxiliary_task == 'crf' else loss_softmax(self.ner_targets)
            loss = srl_loss + ner_loss
        else:
            loss = loss_crf(self.srl_targets) if self.cfg.top_type == 'crf' else loss_softmax(self.srl_targets)
        return loss
    ...
    def train_op(self):
        ...
        grads, _vars = zip(*optimizer.compute_gradients(self.loss))
        grads, norm = tf.clip_by_global_norm(grads, 1.0)
        return optimizer.apply_gradients(zip(grads, _vars), global_step=global_step)
    ...
    def error(self):
        mistakes = tf.not_equal(
            tf.argmax(self.srl_targets, -1), tf.argmax(self.prediction, -1))
    ...
    def restore(self):
        ...
        if self.cfg.tag != "last":
            if self.cfg.tag == "best":
                ...
                saver = tf.train.Saver(max_to_keep=self.cfg.max_to_save, var_list=optimistic_restore_vars(os.path.join(self.cfg.model_dir, self.cfg.tag)))
            ...
            saver = tf.train.Saver(max_to_keep=self.cfg.max_to_save, var_list=optimistic_restore_vars(ckpt.model_checkpoint_path))
            if lst is not None:
    ...
    def train(self):
        ...
        ckpt = tf.train.get_checkpoint_state(os.path.dirname(self.cfg.model_dir + '/checkpoint'))
        saver = tf.train.Saver(max_to_keep=self.cfg.max_to_save, var_list=optimistic_restore_vars(ckpt.model_checkpoint_path) if ckpt is not None else None)
        #TODO: session is a class for running TensorFlow operations
        with tf.Session() as self.sess:
            ...
            for epoch in range(self.my_summary.current_epoch, self.cfg.nb_epochs):
                self.logger.info("Epoch {:} out of {:}".format(epoch, self.cfg.nb_epochs))
                loss, eval = self.run_epoch(train_input, dev_input, epc=epoch)
    ...
    def get_feed(self, inputs_vals, predicate_vals, srl_target_vals, ner_target_vals, seq_len_vals, dr=1.0, char_vals=None, char_lens = None, is_train=True, lr=None, ):
        if lr is None:
            lr = self.cfg.learning_rate
        if ner_target_vals is None:
            if char_vals is not None:
                return {
                    ...
                    self.srl_targets: srl_target_vals,
                    self.dropout: dr,
    ...
    def run_epoch(self, train_input, dev_input, epc=None):
        ...
        while train_input.epochs == current_epoch:
            ...
            (inputs_vals, predicate_vals, srl_target_vals, ner_target_vals), seq_len_vals = train_input.next_batch(self.cfg.batch_size)

            char_vals, char_lens = None, None
            if "char" in self.cfg.emb_type:
                char_vals, char_lens = self.charpp.get_data(inputs_vals)

            feeddct = self.get_feed(inputs_vals, predicate_vals, srl_target_vals, ner_target_vals, seq_len_vals, self.cfg.dropput_keep_prob, char_vals, char_lens, lr=lr)
            _, train_loss = self.sess.run([self.train_op, self.loss], feed_dict=feeddct)
    ...
    def conll_evaluate(self, test_input, test, test_path):
        ...
        while test_input.epochs == current_epoch:
            (inputs_vals, predicate_vals, srl_target_vals), seq_len_vals = test_input.next_batch(self.cfg.batch_dev_size)

            char_vals, char_lens = None, None
            if "char" in self.cfg.emb_type:
                char_vals, char_lens = self.charpp.get_data(inputs_vals)

            feeddct_err = self.get_feed(
            ...
            if self.cfg.top_type == "crf":
                ...
                for score, sequence_length in zip(scoresp.tolist(), seq_len_vals):
                    ...
                    viterbi_seq, viterbi_score = tf.contrib.crf.viterbi_decode(score, trans_params)
                    tmp.append(viterbi_seq)
                all_predictions.extend(tmp)
            else:
                if self.cfg.inference_type == "A*":
                    [score] = self.sess.run([self.logits], feed_dict=feeddct_err)
                    predictions.extend(score.tolist())
                elif self.cfg.inference_type == "argmax":
                    score, predicted_labels = self.sess.run([self.logits, self.prediction], feed_dict=feeddct_err)
    ...
    def get_CoNLL2001Score(self, goldp, sysp):
        child = subprocess.Popen('sh {} {} {}'.format(self.cfg.eval_script, goldp, sysp), shell=True, stdout=subprocess.PIPE)
        eval_info = child.communicate()[0].decode("utf-8")
        try:
            f_score = eval_info.strip().split("\n")[6]
            temps = f_score.strip().split()
